...
/**
 * Minimal dotted-path reader over a YAML file, keeping key parity with the proxy `config.yml`.
 * Geyser's own config API describes Geyser's file, not an extension's, so this loads ours through
 * (relocated) SnakeYAML exactly as the Velocity loader does.
 */
class GeyserConfig private constructor(private val root: Map<String, Any?>) {
    ...
    companion object {
        fun load(file: Path): GeyserConfig {
            if (!Files.exists(file)) return GeyserConfig(emptyMap())
            Files.newInputStream(file).use { input ->
                @Suppress("UNCHECKED_CAST")
                val map = Yaml().load<Any?>(input) as? Map<String, Any?> ?: emptyMap()
                return GeyserConfig(map)
            }
        }
    }
}
