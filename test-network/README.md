...
A self-contained Velocity proxy + two Paper backends + standalone Geyser + Redis + Postgres, wired for a **many-nodes**
...
```
   Java client        Bedrock client
        ...
        +--------------+
           /        \
          v          v
   paper1:25566   paper2:25567     server "prison", many-nodes
          \          /
           v        v
...
| paper2   | `paper2/` (port 25567, id `paper2`)   | Same server, an interchangeable node.               |
...
Postgres only adds persistence (flags/language survive restarts) and clears the "many-nodes but
database.enabled is false" boot banner. To run Redis-only, set `database.enabled: false` in all three configs (`paper1`,
`paper2`, `velocity`), so routing, handoff, maintenance, and flag sync still work,
they just reset on restart.

## Config pointers

- Paper core: `paperN/plugins/Cryon/config.yml` (network.expect, network.server, network.node, redis, database).
- Paper identity is also forced via env in `start-paperN.ps1` (`CRYON_NODE`, `CRYON_SERVER`, `CRYON_EXPECT`) so it can't
