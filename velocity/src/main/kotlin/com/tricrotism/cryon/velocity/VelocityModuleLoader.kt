...
/**
 * The Velocity counterpart to the Paper `ModuleLoader`, over the same `:common` module system, and
 * with the same runtime hot-swap: a feature jar can be added, replaced or removed while the proxy is
 * up. Each jar in `<data>/modules/` is copied into a private cache and loaded from the copy in its own
 * isolated [URLClassLoader], parented to the shared `<data>/api/` contract layer so cross-repo
 * contracts resolve to the same type. Loading from the copy is what keeps the original unlocked, and
 * therefore replaceable, while its modules run.
 *
 * **Single-writer.** Boot drives it on the proxy's init thread; everything afterwards (the `/cryon`
 * command, the watcher) goes through [submit], which is one daemon thread. The proxy has no main
 * thread to funnel onto the way Paper does, so the loader owns that thread itself.
 *
 * One thing does not have Paper's parity, and cannot: proxy commands are Brigadier nodes owned by
 * Velocity's `CommandManager`, which a module withdraws through `VelocityModule.registerCommands`
 * rather than through a core-owned registry.
 */
class VelocityModuleLoader(
    ...
    /**
     * Run [block] on the loader's own thread, the proxy's stand-in for Paper's global region thread.
     */
    ...
    /**
     * Close every loader (modules before the shared parent) and clear the cache. For proxy shutdown.
     */
    fun close() {
        worker.shutdownNow()
