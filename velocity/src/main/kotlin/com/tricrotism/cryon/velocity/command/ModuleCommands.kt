package com.tricrotism.cryon.velocity.command

import com.tricrotism.cryon.common.module.ModuleManager
import com.tricrotism.cryon.velocity.VelocityModuleLoader
import com.tricrotism.cryon.velocity.api.command.Arg
...
/**
 * `/cryon` on the proxy: the module surface Paper has, for the modules the proxy runs. Same verbs,
 * same meanings, so an operator moving between a backend console and the proxy console is not
 * learning a second vocabulary.
 *
 * Every verb that mutates module state runs through [VelocityModuleLoader.submit], the proxy's
 * stand-in for Paper's global region thread: command dispatch and the hot-reload watcher are two
 * different threads, and the loader and manager are single-writer.
 */
@Command("cryon", "Cryon proxy module manager")
@Permission("cryon.admin")
class ModuleCommands(
    private val modules: ModuleManager,
    private val loader: VelocityModuleLoader,
) {
